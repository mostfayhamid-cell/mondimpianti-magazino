...
import { PartWithStock, Person, Transaction, User } from '../types';
...
const WithdrawForm: React.FC<Props> = ({ partsWithStock, people, currentUser, onSubmit, onRepeat }) => {
  const [pn, setPn] = useState('');
  const [qty, setQty] = useState<number>(1);
  const [takenById, setTakenById] = useState(currentUser?.email || '');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      setPn('');
      setQty(1);
      setTakenById(currentUser?.email || '');
      setNote('');
      onRepeat();
    }, 1500);
  };
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                ...
                required
                className="w-full p-3 bg-gray-50 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-red-500 focus:border-red-500 outline-none"
              >
                <option value="">Seleziona dipendente...</option>
                {/* Ensure current user is always an option if logged in */}
                {currentUser && !people.some(p => p.personId.toLowerCase() === currentUser.email.toLowerCase()) && (
                  <option value={currentUser.email}>{currentUser.fullName} (Tu)</option>
                )}
                {people.filter(p => p.active).map(person => (
                  <option key={person.personId} value={person.personId}>
                    {person.fullName} {person.personId.toLowerCase() === currentUser?.email.toLowerCase() ? '(Tu)' : ''}
                  </option>
                ))}
              </select>
            </div>
