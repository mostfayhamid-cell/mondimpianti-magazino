...
export class ErrorBoundary extends React.Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  render() {
    ...
    if (hasError) {
      const isSupabaseError = error?.message.includes('Supabase client used before valid configuration');
        ...
        <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
          <div className="bg-white p-8 rounded-3xl shadow-2xl max-w-lg w-full border border-gray-100">
            ...
            <h1 className="text-2xl font-bold text-gray-900 mb-4 text-center">
              {isSupabaseError ? 'Configurazione Richiesta' : 'Ops! Qualcosa è andato storto'}
            </h1>
            ...
            {isSupabaseError && (
              <div className="space-y-4 mb-8">
                <p className="text-sm font-semibold text-gray-700">Come risolvere:</p>
                <ol className="text-xs text-gray-600 space-y-2 list-decimal pl-4">
                  <li>Apri le <strong>Impostazioni</strong> (icona ingranaggio in alto a destra)</li>
                  <li>Vai nella sezione <strong>Secrets</strong></li>
                  <li>Aggiungi <code>VITE_SUPABASE_URL</code> con l'URL del tuo progetto Supabase</li>
                  <li>Aggiungi <code>VITE_SUPABASE_ANON_KEY</code> con la tua chiave anonima</li>
                  <li>L'app si riavvierà automaticamente</li>
                </ol>
              </div>
            )}
            ...
            <button 
              ...
              className="w-full py-4 bg-indigo-600 text-white font-bold rounded-2xl hover:bg-indigo-700 transition-all shadow-lg active:scale-95"
            >
              Ricarica Pagina
            </button>
          </div>
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (usersData) {
          // De-duplicate by email to handle potential database duplicates
          const uniqueUsers = Array.from(new Map((usersData as any[]).map(u => [u.email.toLowerCase(), u])).values());
          const syncedPeople = uniqueUsers.map((u: any) => ({
            personId: u.email,
            fullName: u.fullName,
            active: true
          }));
      ...
      case View.WITHDRAW:
        return (
          <WithdrawForm 
            ...
            onRepeat={() => setCurrentView(View.WITHDRAW)} 
          />
        );
      case View.PARTS:
        return (
